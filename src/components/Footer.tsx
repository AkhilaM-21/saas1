...
import { fadeUp, fadeUpSmall, fadeLeft, stagger, staggerSlow, scaleIn, viewportOnce } from '../utils/animations';
  ...
  return (
    <footer className="relative bg-white pt-64 md:pt-56 pb-10 mt-16 border-t border-slate-200">
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Floating CTA Card — scales up as it enters */}
        <motion.div
          variants={scaleIn}
          initial="hidden"
          whileInView="visible"
          viewport={{ once: true, margin: '0px' }}
          className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[90%] lg:w-[80%] max-w-5xl bg-navy rounded-[2.5rem] overflow-hidden outline outline-2 outline-primary outline-offset-[6px] md:outline-offset-[12px] shadow-none flex flex-col md:flex-row"
        >
          ...
          <motion.div
            ...
            whileInView="visible"
            viewport={{ once: true }}
            className="w-full md:w-7/12 relative z-10 flex flex-col items-center justify-center px-10 pb-10 md:py-14 md:pr-14 md:pl-0 text-center"
          >
            <motion.h2 variants={fadeUp} className="text-3xl md:text-4xl lg:text-5xl font-extrabold text-white mb-3 md:mb-4 tracking-tight leading-tight">
              {isArabic ? 'هل أنت مستعد لتحويل عملياتك؟' : 'Ready to transform your operations?'}
            </motion.h2>
            <motion.p variants={fadeUpSmall} className="text-white/80 mb-6 max-w-md leading-relaxed font-medium">
            ...
            <motion.button
              ...
              whileHover={{ scale: 1.04, boxShadow: '0 0 30px rgba(214,154,22,0.5)' }}
              whileTap={{ scale: 0.97 }}
              transition={{ duration: 0.2 }}
              className="bg-transparent text-primary border-2 border-primary hover:border-primary-hover px-8 py-3.5 rounded-full font-bold text-[13px] tracking-wide uppercase transition-all flex items-center justify-center gap-2"
            >
        ...
        >
            ...
            <div className="flex gap-4">
              {[Twitter, Linkedin, Globe].map((Icon, i) => (
                <motion.a
                  ...
                  transition={{ duration: 0.2 }}
                  className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 hover:bg-brandblue hover:text-white transition-colors"
                >
          ...
          <motion.div variants={fadeUpSmall}>
            ...
            <ul className="space-y-4 text-slate-500 text-[15px] font-medium">
              {[isArabic ? 'حول' : 'About Us', isArabic ? 'الخدمات' : 'Services', isArabic ? 'المجتمع' : 'Community', isArabic ? 'التوصيات' : 'Testimonials'].map((item) => (
                <li key={item}><a href="#" className="hover:text-brandblue transition-colors">{item}</a></li>
              ))}
            </ul>
          </motion.div>
          ...
          <motion.div variants={fadeUpSmall}>
            ...
            <ul className="space-y-4 text-slate-500 text-[15px] font-medium">
              {[isArabic ? 'مركز المساعدة' : 'Help Center', isArabic ? 'تغريدة لنا' : 'Tweet @ Us', isArabic ? 'ندوات' : 'Webinars', isArabic ? 'ملاحظات' : 'Feedback'].map((item) => (
                <li key={item}><a href="#" className="hover:text-brandblue transition-colors">{item}</a></li>
              ))}
            </ul>
          </motion.div>
          ...
          <motion.div variants={fadeUpSmall}>
            ...
            <ul className="space-y-4 text-slate-500 text-[15px] font-medium">
              <li className="flex items-center gap-3"><Phone className="w-4 h-4 text-brandblue" /><span>+966 50 123 4567</span></li>
              <li className="flex items-center gap-3"><Mail className="w-4 h-4 text-brandblue" /><span>hello@emvive.com</span></li>
              <li className="flex items-center gap-3"><MapPin className="w-4 h-4 text-brandblue" /><span>Riyadh, Saudi Arabia</span></li>
            </ul>
          </motion.div>
        </motion.div>
        ...
        <motion.div
          ...
          whileInView="visible"
          viewport={viewportOnce}
          className="border-t border-slate-200 pt-8 flex flex-col md:flex-row justify-between items-center gap-4 text-slate-400 text-sm font-medium"
        >
