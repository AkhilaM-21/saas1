...
import { fadeLeft, fadeRight, fadeUp, fadeUpSmall, scaleIn, stagger, viewportOnce } from '../../utils/animations';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col lg:flex-row items-center gap-16">
          ...
          >
            ...
            <motion.h2 variants={fadeLeft} className="text-4xl md:text-5xl font-extrabold text-navy mb-6 tracking-tight leading-tight">
              {isArabic ? 'تحكم كامل في عملياتك' : 'Total control over your operations'}
            </motion.h2>
            <motion.p variants={fadeUpSmall} className="text-slate-500 text-lg mb-8 leading-relaxed">
              {isArabic
            ...
            <motion.ul variants={stagger} className="flex flex-col gap-4 mb-10 text-slate-600 font-medium">
              {[
                isArabic ? 'تتبع المخزون المباشر' : 'Live inventory tracking',
                isArabic ? 'رؤى المبيعات والمشتريات' : 'Sales & purchase insights',
                isArabic ? 'إشعارات انخفاض المخزون' : 'Low stock notifications',
                isArabic ? 'تصدير البيانات بنقرة واحدة' : 'Export data in one click.',
              ].map((item) => (
                <motion.li key={item} variants={fadeUpSmall} className="flex items-center gap-3">
            ...
            <motion.button
              variants={fadeUp}
              whileHover={{ scale: 1.04, boxShadow: '0 8px 24px rgba(214,154,22,0.35)' }}
              whileTap={{ scale: 0.97 }}
              transition={{ duration: 0.2 }}
              className="bg-primary hover:bg-primary-hover text-white px-8 py-3.5 rounded-lg font-bold transition-all shadow-md flex items-center justify-center gap-2"
            >
              {isArabic ? 'استكشف لوحة التحكم' : 'Explore Dashboard'} <ArrowRight className="w-5 h-5" />
            </motion.button>
          </motion.div>

          {/* Right Dashboard Mockup — slides from right */}
          <motion.div
            variants={fadeRight}
            initial="hidden"
            whileInView="visible"
            viewport={viewportOnce}
            className="w-full lg:w-2/3 overflow-x-auto hide-scrollbar pb-4 -mb-4"
          >
            <motion.div
              variants={scaleIn}
              initial="hidden"
              whileInView="visible"
              viewport={viewportOnce}
              className="bg-white rounded-2xl shadow-[0_20px_60px_rgba(0,0,0,0.05)] border border-gray-100 flex overflow-hidden h-[500px] min-w-[700px]"
            >
              {/* Sidebar */}
              ...
              <div className="flex-1 bg-white p-6 flex flex-col gap-6">
                ...
                <div className="grid grid-cols-4 gap-4">
                  {[
                    { label: 'Total Items', value: '24,592', color: 'text-navy' },
                    { label: 'Low Stock', value: '14', color: 'text-red-500' },
                    { label: 'Revenue', value: '$84.2K', color: 'text-green-500' },
                    { label: 'Orders', value: '1,892', color: 'text-navy' },
                ...
                <div className="flex gap-4 flex-1 mt-2">
                  <div className="flex-[3] border border-gray-100 rounded-xl p-4 flex flex-col shadow-sm">
                    ...
                    <div className="flex-1 flex items-end justify-between px-2 gap-2 relative">
                      ...
                      {[[60, 20], [80, 10], [50, 40], [90, 5], [70, 30], [100, 10], [40, 60]].map((pair, idx) => (
                  ...
                  <div className="flex-[2] border border-gray-100 rounded-xl p-4 flex flex-col shadow-sm">
                    ...
                    <div className="flex-1 flex items-center gap-4">
                      ...
                      <div className="flex flex-col gap-2">
                        {[['bg-primary', 'Electronics'], ['bg-navy', 'Fashion'], ['bg-amber-200', 'Groceries'], ['bg-slate-200', 'Others']].map(([bg, label]) => (
                          <div key={label} className="flex items-center gap-2 text-[10px] text-slate-500 font-medium">
                            <div className={`w-2 h-2 rounded-full ${bg}`}></div> {label}
                          </div>
                        ))}
                      </div>
                    </div>
